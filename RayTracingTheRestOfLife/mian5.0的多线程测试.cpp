...
void rayTrace(int* nextTileX, int* nextTileY, std::mutex* tileInfoMutex, int tileWidth, int tileHeight, int nx, int ny, int ns, camera cam, intersect& world, char* buffer) {
        ...
        *nextTileX += tileWidth;
        ...
        for (int j = tileY; j < tileY + tileHeight; j++) {
            for (int i = tileX; i < tileX + tileWidth; i++) {
                ...
                for (int s = 0; s < ns; s++) {
                    auto u = (i + random_double()) / float(nx);
                    auto v = (j + random_double()) / float(ny);
                    ray sight = cam.get_ray(u, v);
